...
std::unique_ptr<Parser::AstNode> Parser::buildAST() {
    ...
    while (pos < this->tokens.size()) {
        ...
        if (t.getType() == TokenType::LITERAL) {
            ...
            if (t.getType() == TokenType::AT) {
                ...
                if (!requireToken(TokenType::DOUBLE_COLON, 1)) {
                    throw std::runtime_error(
                        "Invalid step expression: missing '::' after axis "
                        "name");
                }
                pos++;
                if (!requireToken(TokenType::NODE_TYPE, 1) &&
                    ...
                    throw std::runtime_error(
                        "Missing node test after axis name");
                }
                pos++;
                node->test = tokens[pos].getValue();
            }
            ...
            std::unique_ptr<FunctionCall> node =
            ...
            while (!opStack.empty() && opStack.top()->getType() !=
                                           Lexer::TokenType::OPENING_PAREN) {
                applyOperation();
                opStack.pop();
            }
            if (nodeStack.size() < 3) {
            ...
            nodeStack.pop();
            if (FunctionSentinel* fs =
                    dynamic_cast<FunctionSentinel*>(nodeStack.top().get())) {
                nodeStack.pop();
                if (FunctionCall* fc =
                        dynamic_cast<FunctionCall*>(nodeStack.top().get())) {
            ...
            if (dynamic_cast<FunctionSentinel*>(temp.get())) {
                if (nodeStack.empty())
                    throw std::runtime_error("Sentinel without stack");
                if (FunctionCall* fc =
                        dynamic_cast<FunctionCall*>(nodeStack.top().get())) {
                    // sanity check
                    if (!fc->args.empty()) {
                        throw std::runtime_error(
                            "Function should not have no arguments");
                    }
                    function = true;
                } else {
                    throw std::logic_error(
                        "Function Sentinel found without function");
                }
            }

            // otherwise, temp is the last argument and the sentinel is below
            // that
            if (!nodeStack.empty() && !function) {
                if (dynamic_cast<FunctionSentinel*>(nodeStack.top().get())) {
                    nodeStack.pop();
                    if (FunctionCall* fc = dynamic_cast<FunctionCall*>(
                            nodeStack.top().get())) {
            ...
            if (!function) {
                ...
                // This is because some axis have a reverse order, such as
                // ancestor but (ancestor::something) must return to sorted
                // order This information must be kept in the ast
                if (dynamic_cast<Step*>(temp.get())) {
                    std::unique_ptr<FilterExpr> wrapper =
                        std::make_unique<FilterExpr>();
            ...
            if (t.getType() == TokenType::SLASH ||
                t.getType() == TokenType::DOUBLE_SLASH) {
                // If the current token is a slash, the previous token cannot be
                // a slash. We check pos > 0 because if pos == 0, it's a valid
                // absolute path start.
                if (pos > 0) {
                    TokenType prevType = tokens[pos - 1].getType();
                    if (prevType == TokenType::SLASH ||
                        prevType == TokenType::DOUBLE_SLASH) {
                        throw std::runtime_error(
                            "Invalid path syntax: Adjacent slashes.");
                    }
                }
            }

            if (unary && (t.getType() == Lexer::TokenType::SLASH ||
                          t.getType() == Lexer::TokenType::DOUBLE_SLASH)) {
    ...
    if (nodeStack.size() != 1) {
        ...
        throw std::runtime_error(
            "Invalid expression: Unclosed function call or missing operator.");
    }
