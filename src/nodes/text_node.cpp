...
Text::Text(NonOwningNodeTag, std::string text, bool escapeMultiByte)
    : text(text), escapeMultiByte(escapeMultiByte), Node{NonOwning} {}

Text::Text(const Text& other)
    : text(other.text), escapeMultiByte(other.escapeMultiByte), Node{} {}
Text::Text(Text&& other) noexcept
    : text(std::move(other.text)),
      escapeMultiByte(other.escapeMultiByte),
      Node{} {}
...
const std::string& Text::getText() const { return this->text; }
...
bool Text::isVoid() const { return true; }

std::string Text::serialize() const {
    return text::escape(this->text, this->escapeMultiByte);
}

std::string Text::serializePretty(const std::string& indentationSequence,
                                  bool sortAttributes) const {
    return serialize();
}

void Text::specialSerialize(std::vector<Node::SerializationNode>& stack,
                            std::ostringstream& result) const {
    result << text::escape(this->text, this->escapeMultiByte);
    stack.pop_back();
}

void Text::specialSerializePretty(std::vector<Node::SerializationNode>& stack,
...
bool Text::hasSpecialSerialization() const { return true; }

bool Text::shouldEscapeMultiByte() const { return this->escapeMultiByte; }
