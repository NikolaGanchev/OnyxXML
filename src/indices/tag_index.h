 ...
 * provides constant access to a vector of Nodes with any given tag name. The
 * underlying index data structure is an std::unordered_map.
 *
 */
class TagIndex : public Node::Index {
   private:
     ...
     * The key is the tag name, while the value is the std::vector of all
     * indexed Nodes which have that tag name.
     */
   ...
   public:
     ...
     * passed to the method tag name. Returns an empty vector if the Index is
     * invalidated.
     *
     * @param tagName
