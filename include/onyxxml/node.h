 ...
 */
template <typename T>
concept isValidNodeConstructorType =
    std::derived_from<T, Node> || std::same_as<T, Attribute>;

/**
 * @brief Checks if the template parameter is a unique pointer of Node or an
 * Attribute.
 *
 * @tparam T The type
 */
template <typename T>
concept isValidNodeConstructorTypePtr =
...
class Node {
   ...
   private:
    ...
    class ObservableStringRef {
       ...
       public:
        ...
        /**
         * @brief Compares for equality against the underlying std::string to
         * the argument
         *
        ...
        /**
         * @brief Compares for non-equality against the underlying std::string
         * to the argument
         *
     ...
     */
    template <typename T>
    void processConstructorObjectMove(T&& child)
        requires(isNode<T>);

    /**
     * @brief The processConstructorObjectMove(T&&) method can fail, leaving the
     * Node in an invalid state. Cleanup needs to be done for the already
     * allocated nodes.
     *
     */
    ...
    /**
     * @brief Replaces the address of of one index with another without changing
     * any of the index contents. Internal use only.
     *
     * @param oldIndex
     * @param newIndex
     */
     ...
     * during serialization. The default value is "\t".
     */
    static std::string indentationSequence;

    /**
     * @brief Static global state on whether attributes should be sorted during
    ...
    /**
     * @brief Takes over the indices of the Node other by changing their roots.
     * Expects that the indices are already moved to this.
     *
     */
   ...
   public:
    ...
    struct SerializationNode {
        ...
        /**
         * @brief Whether the node has already been passed once by
         * serialization.
         *
         */
        bool visited;
    ...
    /**
     * @brief Methods for special serialization on a Node. Any node subclass
     * returning true on hasSpecialSerialization() must also derive this class.
     *
     */
    class SpecialSerializable {
       ...
       protected:
        /**
         * @brief Called during serialization of a tree for nodes which return
         * true to hasSpecialSerialization(). Temporarily takes over control of
         * the serialization. Must do all serialization actions. Has no
         * obligation towards indentation or formatting, as long as only valid
         * XML is sent to the result stream. Called after
         * SerializationNode::visited is set to true and and if not popped from
         * the stack, when it is encountered again.
         *
         * @param stack The current serialization stack. The top node is the
         * node upon which the method is called. It may choose when and if to
         * pop itself from the stack.
         * @param result The current result stream.
         */
        virtual void specialSerialize(
            std::vector<Node::SerializationNode>& stack,
            std::ostringstream& result) const = 0;

        /**
         * @brief Called during serialization of a tree for nodes which return
         * true to hasSpecialSerialization(). Temporarily takes over control of
         * the serialization. Must do all serialization actions. Must pretty
         * format any XML sent to the output stream. Called after
         * SerializationNode::visited is set to true and and if not popped from
         * the stack, when it is encountered again.
         *
         * @param stack The current serialization stack. The top node is the
         * node upon which the method is called. It may choose when and if to
         * pop itself from the stack.
         * @param result The current result stream. It is guaranteed the result
         * stream will be on the beginning of a newline when received and must
         * be at the beginning of a newline when execution ends.
         * @param indentation The current indentation string. The initial state
         * of this variable will be the correct depth-based indentation string
         * of the current node.
         * @param indentationSequence The indentation string.
         * @param sortAttributes Whether attributes should be sorted.
         */
        virtual void specialSerializePretty(
            std::vector<Node::SerializationNode>& stack,
            std::ostringstream& result, std::string& indentation,
            const std::string& indentationSequence,
            bool sortAttributes) const = 0;
    ...
    /**
     * @brief Construct a new owning Node object via moved arguments. Allows
     * supplying rvalue references to other Node objects of any subclass and
     * Attributes in any order. Validated at compile time. Internally, for Nodes
     * constructs a pointer from the rvalue reference.
     * @tparam Args
     */
    template <typename... Args>
    explicit Node(Args&&... args)
        requires(isValidNodeConstructorType<Args> && ...);

    /**
     * @brief Construct a new owning Node object at runtime. Enforced order of
     * attributes first, then children. Children need to be given over using
     * unique pointers.
     *
     * @param attributes A vector of Attributes
     * @param children A vector of children
     */
    explicit Node(std::vector<Attribute> attributes,
                  std::vector<NodeHandle>&& children);

    /**
     * @brief Construct a new non-owning Node object at runtime. Enforced order
     * of attributes first, then children. Children need to be given over using
     * unique pointers.
     *
     * @param attributes A vector of Attributes
     * @param children A vector of children
     */
    explicit Node(NonOwningNodeTag, std::vector<Attribute> attributes,
                  std::vector<NodeHandle>&& children);
    ...
    /**
     * @brief Destroy the Node object. Calls Index::invalidate() on all indices
     * whose root is this object. Sets isInTree to false for all children.
     *
     */
    virtual ~Node();
    ...
    /**
     * @brief Returns whether the XML tag is void. Void tags are self closing
     * nodes which may not have children.
     *
     ...
     * Searches all children in the tree.
     *
     * @param className
     * @return std::vector<Node*>
     */
    std::vector<Node*> getChildrenByClassName(
        const std::string& className) const;

    /**
     * @brief Get all children that have the given tag name. Searches all
    ...
    /**
     * @brief Get all children that have the given "name" attribute value.
     * Searches all children in the tree.
     *
     * @param name
     * @return std::vector<Node*>
    ...
    /**
     * @brief Get all children that have the given "id" attribute value.
     * Searches all children in the tree.
     *
     * @param id
     * @return std::vector<Node*>
    ...
    /**
     * @brief Get all children that have the given attribute name-value pair.
     * Searches all children in the tree.
     *
     ...
     * @return std::vector<Node*>
     */
    std::vector<Node*> getChildrenByAttribute(const std::string& attribute,
                                              const std::string& value) const;

    /**
     * @brief Get all children that have the given attribute name. Searches all
     * children in the tree.
     *
     * @param attribute The attribute name
     * @return std::vector<Node*>
     */
    std::vector<Node*> getChildrenByAttributeName(
    ...
    /**
     * @brief Get the Attributes vector. Returns a const live reference to the
     * inner Attribute vector of the current Node.
     *
    ...
    /**
     * @brief Get the attribute value by the given name. Throws a runtime error
     * if the attribute name doesn't exist on this node.
     *
     * @param name
     * @return const std::string&
     */
    ...
    /**
     * @brief Set the attribute value by name. If the attribute doesn't exist on
     * the current node, it will be added.
     *
     * @param name
     * @param newValue
     */
    ...
    /**
     * @brief Returns a reassignable constant string reference to the value of
     * the Attribute with the given name. If no such Attribute exists, it is
     * initialised with the empty string "" and a reassignable constant string
     * reference to it is returned.
     *
     * @param name
     * @return ObservableStringRef Behaves as an std::string pointer that can be
     * reassigned, but cannot be modified by std::string method such
     * std::string::append().=
     */
    ObservableStringRef operator[](const std::string& name);

    /**
     * @brief Adds a Node as a child of the current node. Sets the node's
     * isInTree to true. Returns a reference to the node. Throws a runtime error
     * if the current node is void. Throws a runtime error if the owning mode of
     * this isn't the same as that of NodeHandle.
     *
     * @param child
     * @return Node*
     */
    ...
    /**
     * @brief Takes ownership of a Node and adds it as a child of the current
     * node. Sets the node's isInTree to true. Returns a reference to the node.
     ...
     * @brief Adds a Node as a child of the current node. Sets the node's
     * isInTree to true. Returns a reference to the node. Throws a runtime error
     * if the current node is void. Throws a runtime error if this is in owning
     * mode.
     *
     * @param child
     * @return Node*
    ...
    /**
     * @brief Constructs a unique pointer from the given Node and adds it to
     * children. Throws a runtime error if the current node is void.
     *
     * @tparam T
     * @param child
     */
    ...
    /**
     * @brief Replace the given child with a different child. Returns the
     * removed child.
     *
     * @tparam T
     ...
     */
    template <typename T>
    NodeHandle replaceChild(Node* childToReplace, T&& newChild)
     ...
     */
    template <typename T>
    Node& operator+=(T&& right)
        requires(isNode<T>);

    bool operator==(Node& right) = delete;

    /**
     * @brief Removes a child from the current node iteratively. Any child in
     * the tree with the current node as root is searched. If the child is
     * found, is is removed from its parent, isInTree is set to false and
     * ownership is transferred back to the caller via the return value. Returns
     * nullptr if the child is not found.
     *
     * @param childToRemove
     * @return NodeHandle The ownership carrying handle to the removed Node. May
     * be nullptr if the child isn't found.
     */
    ...
    /**
     * @brief Serializes the tree defined by the current node to an XML string
     * with pretty printing. Caution: is slower than serialize(). Performance
     * loss for deep trees and trees with heavily varying depth has been
     * observed to be about 30%.
     *
     * @param indentationSequence The sequence of characters which is used for
     * indentation
     * @param sortAttributes Whether to sort attributes in tags.
     * Attribute order doesn't matter for XML validity or rendering and sorting
     * incurs a runtime cost. In the general case attribute insertion order will
     * be kept by the Node. Attribute sorting is mainly to be used for getting
     * deterministic output for testing.
     *
     * @return std::string
     */
    virtual std::string serializePretty(const std::string& indentationSequence,
                                        bool sortAttributes) const;

    /**
     * @brief Denotes whether a node has special serialization or not. If a node
     * has special serialization, it must also derive from SpecialSerializable.
     * The corresponding SpecialSerializable methods will be called during
     * serialization.
     *
    ...
    /**
     * @brief Creates a shallow copy. A shallow copy copies this node's
     * attributes and other data members. However, tree status, children and
     * indices aren't copied.
     *
    ...
    /**
     * @brief Creates a deep copy. A deep copy copies this node's attributes,
     * children and other data members, but not indices. Caution: expensive
     * operation.
     *
     * @return std::unique_ptr<Node>
     */
    ...
    /**
     * @brief Checks if the attributes and other data members of the two nodes
     * are equal. Does not compare children (except for count), nor indices.
     *
     * @param other
     * @return true
     * @return false
    ...
    /**
     * @brief Checks if the attributes, children and data members of the two
     * nodes are equal. Does not compare indices, nor parents.
     *
     * @param other
     * @return true
     * @return false
